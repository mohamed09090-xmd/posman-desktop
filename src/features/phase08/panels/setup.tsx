...
export function Rules({ gateway, c, setNotice }: { gateway: Gateway; c: Copy; setNotice: (notice: Notice) => void }) {
  ...
  return <div className="p8-split"><form className="p8-form p8-paper" onSubmit={event => void submit(event)}><h3>{c.rules}</h3>
    <Field label="Rule code"><input name="code" defaultValue="RULE-POST" required /></Field><Field label={c.eventType}><select value={eventType} onChange={event => selectTemplate(event.target.value)}>{Object.keys(RULE_TEMPLATES).map(name => <option key={name}>{name}</option>)}</select></Field>
