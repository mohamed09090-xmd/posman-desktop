...
impl Phase05Service {
        ...
        self.open()?.query_row(
            ...
            |row| {
                Ok(CompanyProfile {
                    ...
                    session_idle_timeout_minutes: row.get(20)?, row_version: row.get(21)?,
                })
            },
    ...
    pub fn update_company_profile(&self, request: UpdateCompanyProfileRequest) -> Phase05Result<CompanyProfile> {
        ...
        transaction.execute(
            ...
            "#,
            params![request.default_margin_rate_scaled, request.session_idle_timeout_minutes,
                now_iso()?, context.user_id, context.company_id],
        )?;
