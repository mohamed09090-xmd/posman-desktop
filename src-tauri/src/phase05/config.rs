...
use super::{
    dto::{
        ...
        UpdateCompanyProfileRequest, UpdateDocumentSequenceRequest, UpdateFiscalSetupRequest,
    },
...
impl Phase05Service {
            ...
            "#,
                ...
                |row| {
                    Ok(CompanyProfile {
                        ...
                        legal_form: row.get(6)?,
                        trade_register_number: row.get(7)?,
                        tax_identifier: row.get(8)?,
                        statistical_identifier: row.get(9)?,
                        tax_article_number: row.get(10)?,
                        bank_rib: row.get(11)?,
                        social_capital_minor: row.get(12)?,
                        address_text: row.get(13)?,
                        wilaya_code: row.get(14)?,
                        city: row.get(15)?,
                        postal_code: row.get(16)?,
                        phone: row.get(17)?,
                        email: row.get(18)?,
                        default_margin_rate_scaled: row.get(19)?,
                        below_cost_policy: row.get(20)?,
                        session_idle_timeout_minutes: row.get(21)?,
    ...
    pub fn update_company_profile(
        ...
        if !(0..=1_000_000).contains(&request.default_margin_rate_scaled)
            || !matches!(
                request.below_cost_policy.as_str(),
                "BLOCK" | "ADMIN_OVERRIDE" | "WARNING_ONLY"
            )
    ...
    pub fn get_fiscal_setup(&self) -> Phase05Result<FiscalSetup> {
        ...
        let periods = statement
            .query_map(params![context.company_id, fiscal.0], |row| {
                Ok(FiscalPeriodView {
        ...
        Ok(FiscalSetup {
            fiscal_year_id: fiscal.0,
            code: fiscal.1,
            starts_on: fiscal.2,
            ends_on: fiscal.3,
            periods,
            row_version: fiscal.4,
            in_use,
        })
    }

    pub fn update_fiscal_setup(
        &self,
        ...
        if operational_data_exists(&transaction, &context.company_id)? {
            return Err(Phase05Error::new(
                "FISCAL_YEAR_ALREADY_IN_USE",
                "The fiscal year cannot be changed after operational data exists.",
            ));
        }
        ...
        transaction.execute(
            "DELETE FROM fiscal_periods WHERE company_id=?1 AND fiscal_year_id=?2",
            params![context.company_id, fiscal_year_id],
        )?;
    ...
    pub fn list_document_sequences(&self) -> Phase05Result<Vec<DocumentSequenceView>> {
        ...
        let rows = statement.query_map([context.company_id], |row| {
            ...
            Ok(DocumentSequenceView {
                id: row.get(0)?,
                document_type: row.get(1)?,
                preview: sequence_number(&prefix, &year, next_number, padding),
                prefix,
                next_number,
    ...
    pub fn update_document_sequence(
        ...
        if !(1..=12).contains(&request.padding_width) || request.next_number < 1 {
            return Err(Phase05Error::invalid("documentSequence"));
        }
        ...
        if request.next_number < current.0 {
            return Err(Phase05Error::new(
                "SEQUENCE_NUMBER_DECREASE_FORBIDDEN",
                "The next document number cannot be decreased.",
            ));
        }
        if current.0 > 1 && (prefix != current.1 || request.padding_width != current.2) {
            return Err(Phase05Error::new(
                "SEQUENCE_FORMAT_LOCKED",
                "The sequence format cannot change after the first allocation.",
            ));
        }
        let changed = transaction.execute(
            r#"
        ...
        audit(
            &transaction,
            &context,
            "settings.sequence.update",
            "document_sequences",
            &request.id,
            None,
        )?;
        transaction.commit()?;
        self.list_document_sequences()?
            .into_iter()
            .find(|item| item.id == request.id)
            .ok_or_else(|| Phase05Error::new("NOT_FOUND", "The sequence was not found."))
    }

    #[allow(dead_code)]
    pub(crate) fn allocate_document_number(
        &self,
        fiscal_year_id: &str,
        document_type: &str,
