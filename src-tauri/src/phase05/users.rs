...
impl Phase05Service {
    pub fn list_users(&self, request: PageRequest) -> Phase05Result<Page<UserView>> {
        ...
        for row in rows {
            items.push(UserView {
                role_ids: load_user_role_ids(&connection, &context.company_id, &row.0)?,
                id: row.0,
                username: row.1,
                display_name: row.2,
                preferred_language: row.3,
                is_active: row.4,
                failed_login_count: row.5,
                locked_until: row.6,
                row_version: row.7,
            });
        }
    ...
    pub fn create_user(&self, request: CreateUserRequest) -> Phase05Result<UserView> {
        if request.password != request.password_confirmation
            || !matches!(request.preferred_language.as_str(), "ar" | "fr")
        {
            return Err(Phase05Error::invalid("user"));
        }
    ...
    pub fn update_user(&self, request: UpdateUserRequest) -> Phase05Result<UserView> {
        if !matches!(request.preferred_language.as_str(), "ar" | "fr") {
            return Err(Phase05Error::invalid("preferredLanguage"));
        }
        let context = self.require_session(Some("security.users.manage"))?;
        if request.id == context.user_id && !request.is_active {
            return Err(Phase05Error::new(
                "CURRENT_USER_DEACTIVATION_FORBIDDEN",
                "The current user cannot be deactivated during the active session.",
            ));
        }
        let mut connection = self.open()?;
        if !request.is_active
            && is_system_administrator(&connection, &context.company_id, &request.id)?
            && active_system_administrators(&connection, &context.company_id)? <= 1
        {
        ...
        let changed = transaction.execute(
            ...
            "#,
            params![
                trim_required(&request.display_name, "displayName")?,
                request.preferred_language,
                if request.is_active { 1_i64 } else { 0_i64 },
                now_iso()?,
                context.user_id,
                request.id,
                context.company_id,
        ...
        if !request.is_active {
            transaction.execute("UPDATE sessions SET revoked_at=?1 WHERE user_id=?2 AND company_id=?3 AND revoked_at IS NULL", params![now_iso()?, request.id, context.company_id])?;
        }
        audit(
            &transaction,
            &context,
    ...
    pub fn set_user_roles(&self, request: SetUserRolesRequest) -> Phase05Result<UserView> {
        ...
        if is_system_administrator(&connection, &context.company_id, &request.user_id)?
            && !role_list_contains_system_administrator(
                &connection,
                &context.company_id,
                &request.role_ids,
            )?
    ...
    pub fn reset_user_password(&self, request: ResetUserPasswordRequest) -> Phase05Result<()> {
        ...
        if changed != 1 {
            return Err(Phase05Error::new("NOT_FOUND", "The user was not found."));
        }
    ...
    pub fn list_roles(&self) -> Phase05Result<Vec<RoleView>> {
        ...
        let rows = statement
            .query_map([context.company_id.as_str()], |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
                    row.get::<_, String>(2)?,
        ...
        for row in rows {
            roles.push(RoleView {
                permission_ids: load_role_permission_ids(&connection, &context.company_id, &row.0)?,
                id: row.0,
                code: row.1,
                name_ar: row.2,
                name_fr: row.3,
    ...
    pub fn create_role(&self, request: CreateRoleRequest) -> Phase05Result<RoleView> {
        ...
        transaction.execute(
            ...
            params![
                ...
                trim_required(&request.name_fr, "nameFr")?,
                timestamp,
                context.user_id
            ],
        )?;
    ...
    pub fn update_role(&self, request: UpdateRoleRequest) -> Phase05Result<RoleView> {
        ...
        let system = connection
            ...
            .optional()?
            .ok_or_else(|| Phase05Error::new("NOT_FOUND", "The role was not found."))?
            == 1;
        if system && !request.is_active {
            return Err(Phase05Error::new(
                "SYSTEM_ADMINISTRATOR_ROLE_PROTECTED",
                "The System Administrator role cannot be deactivated or deleted.",
            ));
        }
        let transaction = connection.transaction()?;
    ...
    pub fn set_role_permissions(
        ...
        let mut connection = self.open()?;
        validate_permissions(&connection, &request.permission_ids)?;
        let system: bool = connection.query_row(
            "SELECT is_system FROM roles WHERE id=?1 AND company_id=?2",
            params![request.role_id, context.company_id],
            |row| Ok(row.get::<_, i64>(0)? == 1),
        )?;
        if system {
            let all = all_permission_ids(&connection)?;
            if request.permission_ids.len() != all.len()
                || !all.iter().all(|id| request.permission_ids.contains(id))
            {
                return Err(Phase05Error::new(
                    "SYSTEM_ADMINISTRATOR_PERMISSIONS_PROTECTED",
                    "The System Administrator role must retain every current permission.",
                ));
            }
        }
        let transaction = connection.transaction()?;
...
fn replace_user_roles(
    ...
    for role_id in role_ids {
        transaction.execute("INSERT INTO user_roles (id,company_id,user_id,role_id,assigned_at,assigned_by) VALUES (?1,?2,?3,?4,?5,?6)", params![new_id(), company_id, user_id, role_id, timestamp, actor_id])?;
    }
    Ok(())
}
...
fn replace_role_permissions(
    ...
    for permission_id in permission_ids {
        transaction.execute("INSERT INTO role_permissions (id,company_id,role_id,permission_id,granted_at,granted_by) VALUES (?1,?2,?3,?4,?5,?6)", params![new_id(), company_id, role_id, permission_id, timestamp, actor_id])?;
    }
    Ok(())
}
...
fn validate_roles(
    ...
    for role_id in role_ids {
        if connection
            .query_row(
                "SELECT 1 FROM roles WHERE id=?1 AND company_id=?2 AND is_active=1",
            ...
            .is_none()
        {
            return Err(Phase05Error::invalid("roleIds"));
        }
    }
    Ok(())
}
...
fn validate_permissions(
    ...
    for id in permission_ids {
        if connection
            .query_row("SELECT 1 FROM permissions WHERE id=?1", [id], |_| Ok(()))
            .optional()?
            .is_none()
        {
            return Err(Phase05Error::invalid("permissionIds"));
        }
    }
    Ok(())
}

fn load_user_role_ids(
    connection: &rusqlite::Connection,
    company_id: &str,
    user_id: &str,
) -> Phase05Result<Vec<String>> {
    let mut statement = connection.prepare(
        "SELECT role_id FROM user_roles WHERE company_id=?1 AND user_id=?2 ORDER BY role_id",
    )?;
...
fn active_system_administrators(
    connection: &rusqlite::Connection,
    company_id: &str,
) -> Phase05Result<i64> {
...
fn role_list_contains_system_administrator(
    connection: &rusqlite::Connection,
    company_id: &str,
    role_ids: &[String],
) -> Phase05Result<bool> {
    for role_id in role_ids {
        let code: Option<String> = connection
            .query_row(
                "SELECT code FROM roles WHERE id=?1 AND company_id=?2",
                params![role_id, company_id],
                |row| row.get(0),
            )
            .optional()?;
        if code.as_deref() == Some("SYSTEM_ADMINISTRATOR") {
            return Ok(true);
        }
    }
    Ok(false)
...
fn last_admin_error() -> Phase05Error {
    Phase05Error::new(
        ...
        "The last active System Administrator cannot be deactivated or unassigned.",
    )
}
