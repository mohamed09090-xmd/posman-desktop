...
const PERMISSIONS: &[(&str, &str, &str, bool)] = &[
    ...
    (
        "perm-accounting-configure",
        "accounting.configure",
        "accounting",
        true,
    ),
    (
        "perm-accounting-manual-post",
        "accounting.manual.post",
        "accounting",
        true,
    ),
    (
        "perm-accounting-reverse",
        "accounting.reverse",
        "accounting",
        true,
    ),
    (
        "perm-accounting-period",
        "accounting.period.manage",
        "accounting",
        true,
    ),
    (
        "perm-payment-receipt",
        "payment.receipt.post",
        "payments",
        true,
    ),
    (
        "perm-payment-disbursement",
        "payment.disbursement.post",
        "payments",
        true,
    ),
    (
        "perm-payment-allocate",
        "payment.allocate",
        "payments",
        false,
    ),
    (
        "perm-payment-receipt-allocate",
        "payment.receipt.allocate",
        "payments",
        false,
    ),
    (
        "perm-payment-disbursement-allocate",
...
pub(crate) fn boolean(value: bool) -> i64 {
    if value {
        1
    } else {
        0
...
impl Phase08Service {
    ...
    pub(crate) fn immediate<T>(
        &self,
        operation: impl FnOnce(&Transaction<'_>) -> Phase08Result<T>,
    ) -> Phase08Result<T> {
        let mut connection = self
            .phase05
            .phase06_open()
    ...
    pub(crate) fn read<T>(
        &self,
        operation: impl FnOnce(&rusqlite::Connection) -> Phase08Result<T>,
    ) -> Phase08Result<T> {
        let connection = self
            .phase05
            .phase06_open()
...
pub(crate) fn require_company_row(
    tx: &Transaction<'_>,
    table: &str,
    id: &str,
    company: &str,
    ...
    if tx
        .query_row(&sql, params![id, company], |r| r.get::<_, i64>(0))
        .optional()?
        .is_none()
    {
        return Err(Phase08Error::new(
            code,
            "The requested company-scoped record was not found.",
            false,
        ));
    }
    Ok(())
...
pub(crate) fn require_active_postable_account(
    tx: &Transaction<'_>,
    company: &str,
    id: &str,
) -> Phase08Result<()> {
    let state = tx
        .query_row(
            "SELECT is_active,allow_posting FROM accounts WHERE id=?1 AND company_id=?2",
            params![id, company],
            |r| Ok((r.get::<_, i64>(0)?, r.get::<_, i64>(1)?)),
        )
        .optional()?;
    match state {
        None => Err(Phase08Error::new(
            "ACCOUNT_NOT_FOUND",
            "The configured account was not found for this company.",
            false,
        )),
        Some((0, _)) => Err(Phase08Error::new(
            "ACCOUNT_INACTIVE",
            "The configured account is inactive.",
            false,
        )),
        Some((_, 0)) => Err(Phase08Error::new(
            "ACCOUNT_NOT_POSTABLE",
            "The configured account does not accept postings.",
            false,
        )),
        _ => Ok(()),
    }
