...
macro_rules! command {
    ($name:ident,$request:ty,$result:ty,$method:ident) => {
        #[tauri::command]
        pub async fn $name(state: State<'_, Phase09Service>, request: $request) -> Phase09Result<$result> {
...
command!(phase09_get_template, TemplateKeyRequest, TemplateDetail, get_template);
command!(phase09_create_template_draft, CreateTemplateDraftRequest, TemplateDraftView, create_template_draft);
command!(phase09_update_template_draft, UpdateTemplateDraftRequest, TemplateDraftView, update_template_draft);
command!(phase09_publish_template, PublishTemplateRequest, TemplateVersionView, publish_template);
command!(phase09_retire_template, RetireTemplateRequest, TemplateVersionView, retire_template);
...
pub async fn phase09_preview_document(
    ...
    run(state.inner().clone(), move |service| service.preview_document(&app_handle, request)).await
}

command!(phase09_get_preview_content, String, PreviewContent, get_preview_content);
...
pub async fn phase09_render_document(
    ...
    let app_handle = app.clone();
    run(state.inner().clone(), move |service| service.render_document(&app_handle, request)).await
}

command!(phase09_list_rendered_documents, RenderedDocumentsRequest, Paged<RenderedDocumentView>, list_rendered_documents);
command!(phase09_get_rendered_document, RenderedDocumentKeyRequest, RenderedDocumentView, get_rendered_document);
command!(phase09_verify_rendered_document, RenderedDocumentKeyRequest, RenderedDocumentView, verify_rendered_document);
...
pub async fn phase09_print_rendered_document(
    ...
    let app_handle = app.clone();
    run(state.inner().clone(), move |service| service.print_rendered_document(&app_handle, request)).await
}
...
command!(phase09_list_audit_events, AuditRequest, Paged<AuditEventView>, list_audit_events);
...
query!(phase09_get_backup_settings, BackupSettingsView, get_backup_settings);
command!(phase09_update_backup_settings, UpdateBackupSettingsRequest, BackupSettingsView, update_backup_settings);
command!(phase09_create_backup, CreateBackupRequest, BackupView, create_backup);
command!(phase09_list_backups, BackupListRequest, Paged<BackupView>, list_backups);
command!(phase09_verify_backup, BackupKeyRequest, BackupView, verify_backup);
...
#[tauri::command]
pub async fn phase09_import_backup(
    state: State<'_, Phase09Service>,
...
command!(phase09_restore_backup, RestoreBackupRequest, (), restore_backup);
