...
use super::{
    dto::{
        CreateSalesOrderRequest, EntityResult, IdempotentRequest, SalesOrderActionRequest,
...
fn reserve_order_lines(
    ...
    let rows = statement
        .query_map(params![order_id, context.company_id], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
                row.get::<_, String>(2)?,
    ...
    for (line_id, product_id, warehouse_id, _, remaining) in rows {
        ...
        if current.on_hand - current.reserved < remaining {
