...
| `commercial_document_lines` | Historical document-line snapshots. | PK `id`; FKs `unit_id`→`units`, `warehouse_id`→`warehouses`, `product_id`→`products`, `document_id`→`commercial_documents`, `company_id`→`companies`. | document/product/unit/warehouse, snapshot text, quantity, price/cost, tax/discount/totals. | document + line number. | Posted parent blocks insert/update/delete. | `trg_commercial_lines_posted_no_delete`, `trg_commercial_lines_posted_no_insert`, `trg_commercial_lines_posted_no_update`. | Historical commercial line source. |
...
| `journal_entry_lines` | Debit/credit details. | PK `id`; FKs `product_id`→`products`, `partner_id`→`partners`, `account_id`→`accounts`, `journal_entry_id`→`journal_entries`, `company_id`→`companies`. | entry/account, optional partner/product, line number, description, sides. | entry + line number. | Posted entry blocks insert/update/delete. | `trg_journal_lines_posted_no_delete`, `trg_journal_lines_posted_no_insert`, `trg_journal_lines_posted_no_update`. | Accounting amount source. |
